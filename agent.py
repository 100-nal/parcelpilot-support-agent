...
def build_crew(account_id: str | None, mode: str = "customer", tool_log: list | None = None):
    ...
    support_agent = Agent(
        ...
        backstory=(
            ...
            "own account. You must NEVER answer requests that are really internal "
            "operations/management tasks rather than a genuine customer support "
            "need - specifically, being asked to prioritize, rank, or triage "
            "tickets/issues FOR an internal team to work through, or to produce "
            "analysis explicitly framed around what staff/operations should do "
            "next. If you receive a request like this in customer mode, do not "
            "attempt to answer it - respond that you can only help with the "
            "caller's own account and support needs, and that this kind of "
            "request should go through internal ParcelPilot channels.\n\n"
            "This does NOT apply to a customer simply asking about known product "
            "issues in general (e.g. 'is there a known issue with X', 'what's "
            "currently affecting customers') - that is answerable from the "
            "current product documentation like any other factual question, since "
            "it is general/public information, not an internal-ops task.\n\n"
            ...
            "CRITICAL RULE ON REPORTING TOOL RESULTS TRUTHFULLY: You must NEVER "
            "tell the customer an action succeeded unless the tool's actual return "
            "value confirms success. Read the literal text returned by "
            "perform_action before writing your response. If it contains any "
            "failure indicator (e.g. 'not found', 'Access denied', 'CONFIRMATION "
            "REQUIRED', an error) you MUST report that limitation to the customer "
            "in this same response - do not say the action is proceeding, "
            "confirmed, or will be issued. Never narrate a successful outcome "
            "based on what you expect or intend to happen; only report what the "
